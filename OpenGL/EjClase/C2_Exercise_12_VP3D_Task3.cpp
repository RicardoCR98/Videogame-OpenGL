...
void processInput(GLFWwindow *window)
{
    ...
    float walkingAmplitude = 0.05f; // Ajusta la amplitud del movimiento al caminar
    float walkingFrequency = 5.0f; // Ajusta la frecuencia del movimiento al caminar

    // Calcula el movimiento vertical basado en una función seno
    float yOffset = walkingAmplitude * sin(glfwGetTime() * walkingFrequency);
    //Para W y S
    glm::vec3 forwardMovement = cameraSpeed * cameraFront;
    //Sensacion de movimiento de arriba hacia abajo
    glm::vec3 verticalMovement(0.0f, yOffset * 0.03, 0.0f);

    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
        cameraPos += forwardMovement + verticalMovement;
    if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
        cameraPos -= forwardMovement + verticalMovement;
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
        cameraPos -= glm::normalize(glm::cross(cameraFront, cameraUp)) * cameraSpeed + verticalMovement;
